...
/*
    ...
    Functionality:
        Simple qwerty keyboard implementation with defaults to smaller size for ease of use.
...
var qwerty = {
    ...
    buttonX: 42,
    buttonY: 48,
    buttonXñ: 40,   //width for ñ button (spanish mode)
...
function setString(input) {
    //hide keyboard
    belowFlickTextInput2.visible = false;
    //make other components visible
    miscSett1.visible=true;
    miscSett2.visible=true;
    locationInput.visible = true;
    //set string
    currentCitySettingsText.text= input;
    //reset flickable
    shoppingListFlickAction.height= (ShoppCont.getActionPanelY()-252);
    var empty = "";
    ...
    if((empty.localeCompare(input))==0){
        ...
        //if string input is less than 30 characters long, save it
        if(input.length < 30){
            //testing message
            //console.log("Sring input is <20");
            //save input as string basic type
            qwerty.qwertyStr = input; // was previously qwerty.qwertyS =  input;
            //save to list
